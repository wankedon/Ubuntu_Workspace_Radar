 ...
 * @subsection step1 Step 1: 引用SuperSocketAPI.h和SuperSocket.h，使用SS_InitSSLib()初始化库
 * @subsection step2 Step 2: 设置ip地址和回调函数集合，调用SuperSocket构造函数，创建一个SuperSocket对象
 * @subsection step3 Step 3: 利用该对象发送数据，通过你的回调函数来接收数据
 *
 * @section CppAPI c++ style API使用方法
 * @subsection step1 Step 1: 引用SuperSocketAPI.h，使用SS_InitSSLib()初始化库
 * @subsection step2 Step 2: 设置ip地址和回调函数集合，调用SS_CreateSocket，获得一个fd描述符
 * @subsection step3 Step 3: 利用该fd发送数据，通过你的回调函数来接收数据
 * @subsection step4 Step 4: 不再传输数据时，调用SS_DestroySocket()关闭连接
...
typedef void SS_CONNECT_CALLBACK(void* userPointer);
typedef void SS_DISCONNECT_CALLBACK(void* userPointer);
typedef void SS_SEND_CALLBACK(SSError err, void* userPointer);
typedef void SS_RECEIVE_CALLBACK(const DataBlock&, void* userPointer);

/**
* @brief 回调函数集合
*
*/
struct SSEventCallback
{
	void* userPointer;
